...
class PythonExecTool:
    """Execute arbitrary Python code in a sandboxed subprocess with timeout.

    Useful for complex computations that don't fit calculator or equation_solver:
    matrix operations, number theory, graph algorithms, string processing, etc.

    Common libraries available: math, itertools, collections, functools,
    fractions, decimal, re, string, sympy, numpy (if installed).
    """

    def __init__(self, verbose: bool = True, timeout: int = 30):
        self.verbose = verbose
        self.timeout = timeout

    def call(self, code: str) -> str:
        """Execute Python code and return stdout + the result of the last expression."""
        import subprocess
        import textwrap

        # Wrap code so the last expression is printed
        code = code.strip()
        # Add common imports
        wrapper = textwrap.dedent("""\
            import math, re, string, itertools, collections, functools
            from fractions import Fraction
        ...
        try:
            ...
            if result.stderr:
                ...
                # Filter out warnings, keep actual errors
                error_lines = [l for l in errors.split('\n')
                               if not l.startswith(('Warning', '/')) and l.strip()]
                if error_lines and result.returncode != 0:
                    output += "\nERROR: " + "\n".join(error_lines[-5:])
            if not output:
                output = "(no output — add print() to see results)"
