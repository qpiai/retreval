...
class ValidationTool:
    ...
    def invoke(self, state: dict) -> dict:
        ...
        if expected:
            # Oracle path: gold answer was provided — compare against it.
            ...
            # No reference answer was shown to the agent. Gate retry/backtracking
            # on the agent's own confidence instead of a ground-truth comparison.
            threshold = float(os.getenv('SCORE_PASS_THRESHOLD', '0.7'))
            is_correct = float(state.get('best_score', 0.0)) >= threshold
        ...
        if self.verbose:
            ...
            if expected:
                ...
                print(f"   (self-scored) best_score={float(state.get('best_score', 0.0)):.3f}")
...
class FeedbackLoopController:
    ...
    def __init__(self, memory: Memory = None, verbose: bool = True):
        self.memory = memory
        self.verbose = verbose
...
class BacktrackingTool:
    ...
    def invoke(self, state: dict) -> dict:
        ...
        # Preserve the best answer seen so far — backtracking should not lose it
        prev_answer = state.get('predicted_answer', '')
        if prev_answer:
            best_so_far = state.get('_best_answer_before_backtrack', '')
            best_score_so_far = state.get('_best_score_before_backtrack', 0.0)
            current_score = failing_node.get('combined_score', 0.0)
            if not best_so_far or current_score > best_score_so_far:
                state['_best_answer_before_backtrack'] = prev_answer
                state['_best_score_before_backtrack'] = current_score
